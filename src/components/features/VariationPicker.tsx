      ...
      <div className="bg-surface w-full max-w-[900px] border border-border rounded-sm shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="h-12 bg-surface-secondary border-b border-border flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-text-primary text-sm tracking-wide uppercase">Select Variation</span>
            <span className="bg-surface border border-border text-text-secondary text-[10px] px-2 py-0.5 rounded-sm font-mono">{candidates.length} OPTIONS</span>
          </div>
          <button onClick={onCancel} className="p-1 hover:bg-surface rounded text-text-tertiary hover:text-text-primary transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 media-bg">
          <div className={`grid gap-4 h-full ${candidates.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
            {candidates.map((img, idx) => (
              <div key={idx} className="relative group rounded-sm overflow-hidden bg-surface-secondary transition-all flex flex-col border border-border hover:border-primary">
                ...
                <div className="h-8 bg-surface-secondary border-t border-border flex items-center justify-between px-3">
                  <span className="text-[10px] font-mono text-text-muted">OPTION {idx + 1}</span>
                  <button
                    onClick={(e) => { e.stopPropagation(); onRegenerateSlot(idx); }}
                    disabled={generatingSlotIndex !== null}
                    className="text-[10px] flex items-center gap-1 text-text-secondary hover:text-primary disabled:opacity-50 transition-colors"
        ...
        {/* Footer Actions */}
        <div className="h-14 bg-surface-secondary border-t border-border flex items-center justify-between px-4 shrink-0">
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              className="px-4 py-2 border border-border bg-surface hover:bg-surface-hover text-text-secondary rounded-sm text-xs font-bold uppercase tracking-wide transition-colors"
            >
              Cancel
            </button>
            <button
