...
export const ExportPreviewRenderer: React.FC<ExportPreviewRendererProps> = ({
    ...
    const pages = useMemo(() => {
        ...
        // Use the robust Pagination Engine (Source of Truth)
        // This ensures the preview matches the PDF exactly (splits, MORE, CONT'D)
        const paginatedPages = paginateScript(elements);
        return paginatedPages.map(p => p.elements);
    }, [elements, project.id]);
    ...
    const renderSingleElement = (element: ScriptElement, isDual: boolean, isFirstOnPage: boolean = false) => {
        ...
        // Add (CONT'D) to character names (Now derived from Metadata)
