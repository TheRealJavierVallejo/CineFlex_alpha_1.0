...
export const PageWithToolRail: React.FC<PageWithToolRailProps> = ({
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-black relative">
      ...
      <div className="w-[50px] bg-[#050505] border-r border-border flex flex-col items-center py-4 gap-4 shrink-0 z-30">
        {tools.map(tool => (
          <button
            key={tool.id}
          ...
          >
            ...
            {/* Tooltip on Hover */}
            <div className="absolute left-full ml-2 px-2 py-1 bg-zinc-800 text-white text-[10px] font-bold uppercase tracking-wide rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity">
              {tool.label}
            </div>
          </button>
        ))}
      ...
      <div 
        className={`
          relative border-r border-border bg-[#09090b] flex flex-col transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] z-20
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative min-w-0 bg-black z-10">
         {children}
      </div>
    </div>
  );
