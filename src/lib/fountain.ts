// Ported from fountain-js 0.1.10
// Original Author: Matt Daly
// Modernized for CineFlex Studio
...
export interface FountainOutput {
    title: string;
    html: {
        title_page: string;
        script: string;
...
const tokenize = (script: string): FountainToken[] => {
    ...
    while (i--) {
        ...
        if (regex.title_page.test(line)) {
            const matchArr = line.replace(regex.title_page, '\n$1').split(regex.splitter).reverse();
            for (x = 0, xlen = matchArr.length; x < xlen; x++) {
                parts = matchArr[x].replace(regex.cleaner, '').split(/\:\n*/);
                tokens.push({
                    type: parts[0].trim().toLowerCase().replace(' ', '_') as any,
                    text: parts[1].trim()
                });
            }
            continue;
        ...
        if ((match = line.match(regex.scene_heading))) {
            ...
            if (text.indexOf('  ') !== text.length - 2) {
                ...
                tokens.push({ type: 'scene_heading', text: text, scene_number: meta ? (meta as any) : undefined });
            }
            continue;
        }
...
const inlineLexer = (s: string) => {
    ...
    s = s.replace(regex.note_inline, inlineRules.note)
        .replace(/\\\*/g, '[star]')
        .replace(/\\_/g, '[underline]')
        .replace(/\n/g, inlineRules.line_break);
    ...
    while (i--) {
        ...
        if (match.test(s)) {
            s = s.replace(match, (inlineRules as any)[style]);
        }
    }
